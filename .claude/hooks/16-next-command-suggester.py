...
COMMAND_FLOWS = {
    ...
    'generate-tasks': {
        'orchestratable': [
            ...
            {'cmd': '/wt {features} --from-tasks', 'reason': 'Use worktrees for true parallel execution'},
    ...
    'orchestrate-agents': [
        ...
        {'cmd': '/wt --from-tasks', 'reason': 'Consider worktrees for file isolation'},
    ...
    'analyze-existing': [
        {'cmd': '/chain eps', 'reason': 'Run complete integration workflow'},
        {'cmd': '/sr', 'reason': 'Load all context after integration'},
        {'cmd': '/tl', 'reason': 'View extracted tasks'}
    ],
    'log-decision': [
        {'cmd': '/python-import-updater', 'reason': 'Update imports after changes'},
        {'cmd': '/checkpoint', 'reason': 'Save after architectural decision'},
        {'cmd': '/test', 'reason': 'Verify changes work'}
    ],
    'roadmap-view': [
        {'cmd': '/tl view {phase}', 'reason': 'Drill into specific phase'},
        {'cmd': '/pt {next_feature}', 'reason': 'Work on next priority'},
        {'cmd': '/decision review', 'reason': 'Review recent decisions'}
    ],
    # Worktree Commands
    'worktree-parallel': [
        {'cmd': '/wt-status --monitor', 'reason': 'Monitor worktree progress'},
        {'cmd': '/wt-list', 'reason': 'See all active worktrees'},
        {'cmd': '/chain wte', 'reason': 'Execute tasks in worktrees'}
    ],
    'worktree-status': [
        {'cmd': '/wt-review {name}', 'reason': 'Review changes in worktree'},
        {'cmd': '/wt-merge {name}', 'reason': 'Merge completed worktree'},
        {'cmd': '/wt auth claude', 'reason': 'Open Claude in specific worktree'}
    ],
    'worktree-list': [
        {'cmd': '/wt {feature1} {feature2}', 'reason': 'Create new worktrees'},
        {'cmd': '/wt-status', 'reason': 'Check detailed status'},
        {'cmd': '/wt-clean', 'reason': 'Remove completed worktrees'}
    ],
    'worktree-review': [
        {'cmd': '/chain mpr', 'reason': 'Multi-perspective review'},
        {'cmd': '/wt-merge {name}', 'reason': 'Merge after review'},
        {'cmd': '/wt-status', 'reason': 'Check other worktrees'}
    ],
    'worktree-merge': [
        {'cmd': '/wt-clean {name}', 'reason': 'Clean up merged worktree'},
        {'cmd': '/wt-status', 'reason': 'Check remaining worktrees'},
        {'cmd': '/pr', 'reason': 'Create pull request'}
    ],
    # Multi-Perspective Review
    'review-perspectives': [
        {'cmd': '/pr-feedback --comprehensive', 'reason': 'Generate comprehensive report'},
        {'cmd': '/chain pmr', 'reason': 'Full PR review workflow'},
        {'cmd': '/grade', 'reason': 'Check implementation quality'}
    ],
    'multi-perspective-review': [
        {'cmd': '/pr-feedback', 'reason': 'Create PR with review insights'},
        {'cmd': '/wt-merge {name}', 'reason': 'Merge after positive review'},
        {'cmd': '/fix-issues', 'reason': 'Address review findings'}
    ]
}
...
DECISION_CONTEXTS = {
    ...
    'existing_project': [
        {'cmd': '/analyze-existing', 'reason': 'Analyze and integrate boilerplate'},
        {'cmd': '/chain eps', 'reason': 'Full integration workflow'},
        {'cmd': '/help existing', 'reason': 'Learn about integration options'}
    ]
}
...
def analyze_command_result(cmd_name: str, result: Dict, context: Dict) -> str:
    ...
    if any(word in output_lower for word in ['existing project', 'existing codebase', 'drop into', 'integrate']):
